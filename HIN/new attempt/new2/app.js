document.addEventListener('DOMContentLoaded', function() {
    ...
    function prepareSafetyData(geojson) {
        ...
        return {
            ...
            features: geojson.features.map(feature => {
                ...
                // Enhanced safety score calculation (0-1 where 1 is safest)
                let safetyScore = totalCrashes === 0 ? 1.0 : 1 / (1 + Math.log1p(annualCrashes * 3));
    ...
    // =============================================
    // 2. Routing functionality with GraphHopper
    // =============================================
    
    // GraphHopper API key - prompt user if not set
    let graphhopperKey = localStorage.getItem('graphhopperKey');
    if (!graphhopperKey) {
        graphhopperKey = prompt("Please enter your GraphHopper API key:");
        if (graphhopperKey) {
            localStorage.setItem('graphhopperKey', graphhopperKey);
        } else {
    ...
    // Initialize routing control with GraphHopper
    var routingControl = L.Routing.control({
        ...
        router: new L.Routing.GraphHopper(graphhopperKey, {
            urlParameters: {
                vehicle: 'car',
                elevation: false
            }
    ...
    // Show loading when routing starts
    ...
    function setupButtons() {
        ...
        document.querySelectorAll('.route-option').forEach(button => {
            button.addEventListener('click', function() {
                ...
                // Update GraphHopper parameters based on route type
                const router = routingControl.getRouter();
                if (this.dataset.route === 'safest') {
                    router.options.urlParameters.weighting = 'shortest';
                } else {
                    router.options.urlParameters.weighting = 'fastest';
                }
